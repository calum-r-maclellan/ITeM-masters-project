...
x = data(1:nSurg,1:8);
    ...
    [m,~] = size(x); % need to randomise m=80 rows
    rndise = randperm(m); 
    % tells me where the surgeries have moved to. for example, if the rndise(1,1) is 
    % 31 this means that the first surgery x(1,:) has been moved to X(31,1). 
    X = x;
    X(rndise,1:8) = x(:,1:8);

    % Found that randomising the data did nothing to the weights. They
    % maintained the following values:
    % beta0 = -959.1044
    % beta1 = 0.5735
    % beta2 = 0.3718
    % beta3 = 0.1729
    ...
    % beta6 = 0.2832
    % beta7 = 0.2459
    % Try reducing the number of surgeries and see how the weights are
    % affected.
    % => still no difference: for n=30 surgeries the weights dont seem to
    %    be any different, despite 30 different surgeries being evaluated.
    % --------------------
...
% overall results or weight values.
...
legend('\beta_1','\beta_2','\beta_3','\beta_4','\beta_5','\beta_6','\beta_7','location','best');
title('Effect of increasing input size on regression coefficients');
xlabel('\bfNumber of surgeries included');
ylabel('\bfParameter Value');
...
xlabel('\bfSurgeries used');
ylabel('\bfParameter value');
axis([0.75 10.25,-4 15]);
...
for p = 1:7
    meanA(p) = mean(A(p,:));
    stdA(p)  = std(A(p,:));
    errorbar(phases(p),meanA(p),stdA(p),'bx');
    axis([.5 7,0.5 1]);
end
